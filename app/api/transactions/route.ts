...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "發生錯誤" }, { status: 500 })
  }
}
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        notificationSent:
          subscriptions.length > 0 && !!process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY && !!process.env.VAPID_PRIVATE_KEY,
      },
    })
  } catch (error) {
